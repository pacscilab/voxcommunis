import re, csv, subprocess, unicodedata, os
import pandas as pd
import numpy as np
...
def is_cyrillic_letter(char):
    ...
    return char.isalpha() and 'а' <= char.lower() <= 'я'
...
def is_german_word(word):
    ...
    return all(char.lower() in german_letters for char in word)
...
def is_vietnamese(word):
    ...
    for char in word:
        if char.lower() not in vietnamese_characters:
            return False
    return True
